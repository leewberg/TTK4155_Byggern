...
void oled_goto_line(uint8_t line) {
	SPI_select_slave(OLED_CMD);
	// some bitmagic here: 0xB0 is the prefix for the command, the lower 4 bits
	// are the line number and they are masked with & 0x0F
...
void oled_goto_col(uint8_t col) {
	SPI_select_slave(OLED_CMD);
	// some more bitmagic: we have to split the 8 bit column number into two 4
	// bit parts and send them with different commands
...
void oled_print(const char *string) {
	while (*string) {
		char c = *string++; // returns current char from string and increments
							// the pointer
		for (uint8_t i = 0; i < 8; i++) {
			oled_write_data(pgm_read_byte(
				&font8[c - 32][i])); // reads the byte from PROGMEM and sends it
									 // to the oled
		}
	}
}
