  ...
  for (x = 0; x < SCREEN_WIDTH; x += RES_DIVIDER) {
    ...
    if (hit) {
      ...
      drawVLine(x, view_height / distance - line_height / 2 + RENDER_HEIGHT / 2,
                ...
                GRADIENT_COUNT -
                    (uint8_t)(distance / MAX_RENDER_DEPTH * GRADIENT_COUNT) -
                    side * 2);
    }
  }
}
...
Coords translateIntoView(Coords *pos) {
  ...
  Coords result;
  result.x = transform_x;
  result.y = transform_y;
  return result;
}
...
void renderEntities(double view_height) {
  ...
  for (uint8_t i = 0; i < num_entities; i++) {
    ...
    case E_ENEMY: {
      ...
      if (entity[i].state == S_ALERT) {
        // walking
        sprite = (uint8_t)(systick_ms / 500) % 2;
      } else if (entity[i].state == S_FIRING) {
        // fireball
        sprite = 2;
      } else if (entity[i].state == S_HIT) {
      ...
      drawSprite(sprite_screen_x - BMP_IMP_WIDTH * .5 / transform.y,
                 ...
                 BMP_IMP_WIDTH, BMP_IMP_HEIGHT, sprite, transform.y, false);
      break;
    }
    ...
    case E_FIREBALL: {
      drawSprite(sprite_screen_x - BMP_FIREBALL_WIDTH / 2 / transform.y,
                 ...
                 BMP_FIREBALL_HEIGHT, 0, transform.y, false);
      break;
    }
...
void renderGun(uint8_t gun_pos, double amount_jogging) {
  // jogging
  char x = 48 + sin((double)systick_ms * JOGGING_SPEED) * 10 * amount_jogging;
  char y = RENDER_HEIGHT - gun_pos +
           abs(cos((double)systick_ms * JOGGING_SPEED)) * 8 * amount_jogging;

  if (gun_pos > GUN_SHOT_POS - 2) {
    // Gun fire
	drawSprite(x + 6, y - 11, bmp_fire_bits, NULL, BMP_FIRE_WIDTH, BMP_FIRE_HEIGHT, 0, 1.0, true);
  ...
  drawSprite(x, y, bmp_gun_bits, bmp_gun_mask, BMP_GUN_WIDTH, BMP_GUN_HEIGHT, 0, 1.0, false);
}
...
void updateHud() {
  clear_rect(12, 58, 15, 6);
  clear_rect(50, 58, 5, 6);

  drawInt(12, 58, player.health);
  drawInt(50, 58, player.keys);
}

// Debug stats
void renderStats() {
  clear_rect(58, 58, 70, 6);
  drawInt(114, 58, (uint16_t)(getActualFps()));
  drawInt(82, 58, num_entities);
...
void loopIntro() {
  ...
  // drawSprite((SCREEN_WIDTH - BMP_GUN_WIDTH) / 2,
  // 	 SCREEN_HEIGHT - BMP_GUN_HEIGHT, bmp_logo_bits, NULL,
  // 	 BMP_GUN_WIDTH, BMP_GUN_HEIGHT, 0, 1.0, false);
...
void loopGamePlay() {
  ...
  do {
    ...
    if (player.health > 0) {
      // Player speed
      if (input_data->up) {
        player.velocity += (MOV_SPEED - player.velocity) * .4 * input_data->joy_y / 100.0;
      ...
      // Player rotation
      if (input_data->right) {
        rot_speed = ROT_SPEED * delta * input_data->joy_x / 100.0;
      ...
      view_height = abs(sin((double)systick_ms * JOGGING_SPEED)) * 6 * jogging;
      ...
      if (gun_pos > GUN_TARGET_POS) {
        ...
        // Showing up
        gun_pos += 2;
      } else if (!gun_fired && input_data->fire) {
	...
	can_transmit();
	// CAN communication stuff. here goes update input data for instance
...
void loop(void) {
  ...
  case INTRO: {
    loopIntro();
	// loopGamePlay(); // Directly to game play for debug
    break;
  }
  case GAME_PLAY: {
	printf("Starting game play\r\n");
    loopGamePlay();
    break;
  }
  }
