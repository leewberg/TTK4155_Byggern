...
typedef struct {
	...
	POS joy_pos; // same as raw_data from adc. might do the same processing if we decide to use it
	POS touch_pos; // ditto
	uint8_t right_btns; //total of 6 buttons, the two MSBs do nothing
	uint8_t left_btns; //total of 7 buttons, MSB does nothing
	uint8_t nav_switch; //bit 0: up, bit 1: down, bit 2: left, bit 3: right, bit 4: button
} INPUT_DATA;
